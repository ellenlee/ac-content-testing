...
若客戶只購買其中一集的哈利波特，則總價不打折；而購買其中一集的情境，又可以區分成只購買一集 1 本和一集多本，因此測試案例會如下所示：
...
若客戶購買兩本不同集的哈利波特，則會打 5% 的折扣；而購買兩集打折的測試案例，可以有三種，若兩本不同集的哈利波特為一組，三種測試案例會分別是：
- 買一組：1 本第一集、1 本第二集的哈利波特
- 買一組 + 一本：1 本第一集、2 本第二集的哈利波特
- 買兩組：2 本第一集、2 本第二集的哈利波特
...
小撇步：你可以在 `it` 內的字串寫上你輸入的資料，這樣在測試時會更容易檢索自己輸入了什麼資料，如第一個測試案例的 `it`：`第一集買 1 本 : [4]`。
